        ...
        private void tbSearchSearch_TextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                while(rdr.Read())
                {
                    ...
                    tbResult.AppendText(" "); //remove the - 1 in select if this is removed, this prevents all the text from being highlighted

                    int startIndex = word.IndexOf(searchText, StringComparison.OrdinalIgnoreCase);
                    if (startIndex >= 0)
                    {
                        tbResult.Select(tbResult.Text.Length - word.Length - 1, searchText.Length);
